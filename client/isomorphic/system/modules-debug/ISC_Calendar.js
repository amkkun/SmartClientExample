...
/*
  ...
  Version v8.3p_2013-06-16/LGPL Deployment (2013-06-16)
...
if (window.isc && isc.version != "v8.3p_2013-06-16/LGPL Deployment") {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v8.3p_2013-06-16/LGPL Deployment'. Mixing resources from different "
...
getVisibleStartDate : function () {
    ...
    if (selectedView == this.monthView) return this._getDateFromPoint(0,0);
    return this._getDateFromPoint(0,1);
},
...
getVisibleEndDate : function () {
    ...
    return this._getDateFromPoint(selectedView.getData().length-1, selectedView.getFields().length-1);
},
...
// @value "week" events are fetched one week at a time.  Month view may not be used
...
addEvent : function (startDate, endDate, name, description, otherFields, ignoreDataChanged, laneName) {
    ...
    var _finish = function (dsResponse, data, dsRequest) {
        ...
        // show errors inline in the appropriate UI
        var errors = dsResponse ? dsResponse.errors : null,
            fromDialog = self._fromEventDialog,
            fromEditor = self._fromEventEditor;

        delete self._fromEventDialog;
        delete self._fromEventEditor;

        if (errors) {
            if (fromDialog) {
                self.eventDialog.items[0].setErrors(errors, true);
                self.eventDialog.show();
            } else if (fromEditor) {
                self._showEventEditor(newEvt);
                self.eventEditor.setErrors(errors, true);
            }
            return;
        }
    ...
    if (this.dataSource) {
        isc.DataSource.get(this.dataSource).addData(evt, _finish, {componentId: this.ID, willHandleError: true});
        return;
    } else {
...
updateEvent : function (event, startDate, endDate, name, description, otherFields, ignoreDataChanged, laneName) {
    ...
    if (this.dataSource) {
        ...
        ds.updateData(updatedRecord, _finish, {oldValues: event, componentId: this.ID, willHandleError: true});
        return;
    } else {
...
createChildren : function () {
            ...
            isc.DynamicForm.create({
                ...
                createFields : function (isEvent) {
                    var nameType = isEvent ? "staticText" : "text",
                        laneType = isEvent ? "staticText" : "select",
                        cal = this.calendar
                    ;
                    ...
                    if (isEvent) fieldList.removeAt(2);
    ...
    this.eventEditor = this.createAutoChild("eventEditor", {
        ...
        initWidget : function () {
            ...
            if (cal.isTimeline() && cal.canEditLane) {
                var laneMap = cal.getLaneMap(),
                    field = { name: cal.laneNameField, title: cal.eventLaneFieldTitle,
                        type: "select",
            ...
            if (editStyle == "date" || editStyle == "datetime") {
                fieldList.addList([
                    { name: "startDate", title: cal.eventStartDateFieldTitle, type: editStyle, endRow: true },
                    { name: "endDate", title: cal.eventEndDateFieldTitle, type: editStyle, endRow: true },
                    { name: "invalidDate", type: "blurb", colSpan: 4, visible: false,
            ...
            fieldList.addList([
                {name: "name", title: cal.eventNameFieldTitle, type: "text", colSpan: "*", width: "*"},
                {name: "description", title: cal.eventDescriptionFieldTitle, type: "textArea", colSpan: "*",
                    width: "*", height: 50}
            ]);
        ...
        _internalFields : ["startHours", "startMinutes", "startAMPM", "endHours",
                "endMinutes", "endAMPM", "name", "description", "startDate", "endDate"],
        getCustomValues : function () {
    ...
    this.eventEditorLayout = this.createAutoChild("eventEditorLayout", {
        ...
        setDate : function (startDate, endDate, eventName) {
            ...
            ;
            if (editStyle == "date" || editStyle == "datetime") {
                form.setValue("startDate", startDate.duplicate());
                form.setValue("endDate", endDate.duplicate());
            } else if (editStyle == "time") {
        ...
        setEvent : function (event) {
            ...
            form.setValue("name", event[cal.nameField]);
            form.setValue("description", event[cal.descriptionField]);
...
setDateLabel : function () {
    ...
    this.dateDisplay.setContents(content);
},

//> @method calendar.getDateLabelText()
// Returns the text to display with the navigation buttons above the Calendar - indicates the
// visible date range.
...
_getCellCSSText : function (grid, record, rowNum, colNum) {
    var cal = this,
        currDate = cal._getDateFromPoint(rowNum, colNum),
...
_getDateFromPoint : function (rowNum, colNum) {
    ...
    if (this.dayViewSelected()) {
        ...
        var field = this.timelineView.body.getField(colNum);
...
getPrintHTML : function (printProperties, callback) {
    ...
    if (isTimeline) {
        //isc.logWarn(isc.echoFull("widths are: " + fields.getProperty("width")));
        fieldWidths = fields.getProperty("width");
...
isc.DaySchedule.addProperties({
    ...
    cellMouseDown : function (record, rowNum, colNum) {
        ...
        var startDate = this.creator._getDateFromPoint(this.getEventRow(), this.getEventColumn());
    ...
    cellMouseUp : function (record, rowNum, colNum) {
        ...
        if (this.creator.backgroundClick) {
            var startDate = this.creator._getDateFromPoint(sRow, colNum);
            var endDate = this.creator._getDateFromPoint(sRow+diff, colNum);
...
isc.MonthSchedule.addProperties({
    ...
    cellHoverHTML : function (record, rowNum, colNum) {
        ...
        if (!this.rowIsHeader(rowNum) && record["event" + fieldIndex] != null) {
            ...
            var retVal = "";
            for (var i = 0; i < evtArr.length; i++) {
                var eTime = isc.Time.toTime(evtArr[i][this.creator.startDateField], this.creator.timeFormatter, true);
...
isc.EventWindow.addProperties({
    ...
    getPrintHTML : function (printProperties, callback) {
        var html = this.Super("getPrintHTML", arguments);
        return html;
    },
    ...
    updateColors : function () {
        var cal = this.calendar,
            event = this.event,
            header = this.header,
            labelParent = header && header.getMember ? header.getMember(0) : null,
            label = labelParent
        ;
    ...
    getPrintHTML : function (printProperties, callback) {
        ...
        if (cal.showEventDescriptions) {
            output.append(this.title, "<br>", this.event.description);
        } else {
    ...
    dragRepositionStop : function () {
        ...
        var EH = this.ns.EH,
            ...
            rowNum = Math.max(0, Math.floor(dragTop / rowHeight)), //this.parentElement.getEventRow(),
...
isc.TimelineWindow.addProperties({
    ...
    click : function () {
        var cal = this.calendar;
        var doDefault = cal.eventClick(this.event, "timeline");
        if (doDefault) {
            if (!cal.canEditEvent(this.event)) return;

            var col = cal.timelineView.getEventColumn(this.getLeft() + 1),
                row = cal.timelineView.getTimelineEventRowNum(this.event)
            ;
...
isc.TimelineView.addProperties({
    ...
    timelineDragTargetDefaults: {
        ...
        positionToEventWin : function (show) {
            var eventWin = this.eventWin,
                left = this.timelineView.getEventLeft(eventWin.event),
                top = eventWin.getTop(),
        ...
        dragRepositionStart : function () {
            var eventWin = this.eventWin,
                ...
                eventLeft = this.timelineView.getEventLeft(eventWin.event),
                eventCol = gr.getEventColumn(), //eventLeft),
                offsetX = gr.getOffsetX() - this.eventWin.getLeft()
            ;

            eventWin._startRow = eventRow;
            eventWin._mouseColDelta = (gr.getEventColumn() - eventCol) - 1;
            ...
            return isc.EH.STOP_BUBBLING;
            //isc.logWarn("INITIAL eventCol is " + eventCol);
        },
        dragRepositionMove : function () {
            ...
            //isc.logWarn("CURRENT eventCol is now " + eventCol);
        ...
        dragRepositionStop : function () {
            ...
            var dragLeft = EH.dragMoveTarget.getLeft();
            ...
            if (!cal.timelineEventMoved(event, eventWin, dates[0], dates[1], newLane)) return false;

            // hide the manual dragTarget
            this.hide();
        ...
        dragResizeStop : function () {
            ...
            if (!cal.timelineEventResized(event, eventWin, startDate, endDate)) return false;
    ...
    initWidget : function () {
        ...
        if (c[c.startDateField]) this.startDate = c[c.startDateField];
        if (c[c.endDateField]) this.endDate = c[c.endDateField];
        ...
        if (!this.startDate) {
            this.startDate = c[c.startDateField] = isc.DateUtil.getAbsoluteDate("-0" + granString, c.chosenDate);
        }

        if (!this.endDate) {
            // no endDate - default to defaultTimelineColumnSpan columns of timelineGranularity
            this.endDate = c[c.endDateField] = isc.DateUtil.getAbsoluteDate("+" +
            ...
            this.startDate = c[c.startDateField] = this.endDate;
            this.endDate = c[c.endDateField] = s;
    ...
    getVisibleEvents : function () {
        ...
        for (var i = 0; i < events.getLength(); i++) {
           ...
           // isc.logWarn('eventRowIndex:' + eventRowIndex);
            ...
            if (this.eventsOverlap(rangeObj, event, (cal.canEditLane == true))
                && rowRange[0] <= eventRowIndex
    ...
    tagDataForOverlap : function (data, lane) {
        ...
        var tl = this,
            ...
            firstEvent = data.get(0), // the first event in the passed data
        ...
        data.sort(function (a, b) {
            ...
            if (a[cal.laneNameField] < b[cal.laneNameField]) return -1;
            else if (a[cal.laneNameField] > b[cal.laneNameField]) return 1;
            else {
                // allow user to cusomize the secondary sort, useful when allEventsOverlap
        ...
        //cal.data.sort(comparator); // sort by event lane, start date
        for (var i = start; i < data.getLength(); i++) {
            var event = data.get(i);
            //isc.logWarn("tagDataForOverlap, processing:" + event.EVENT_ID);
            // stop processing if we've already processed all events w/ parameter lane
            if (lane && event[cal.laneNameField] != lane) {
                break;
            } else if (lane) processedEvents.add(event);
            // reset overlap vars when a new lane is encountered
            if (event[cal.laneNameField] != currLane) {
            //    isc.logWarn('resetting - was ' + currLane + " and is now " + event[cal.laneName]);
                overlapMembers = currentOverlapTot = maxTotalOverlaps = 0;
                priorOverlaps = [];
                currLane = event[cal.laneNameField];
            }

            var overlapsAll = true;
            var keepers = [];
            //if(lane)isc.logWarn('start processing:' + [event.assayEventId, currentOverlapTot, event.startDate]);
            ...
            for (var j = 0; j < priorOverlaps.length; j++) {
                ...
                if (!isc.isAn.Object(overlapper._overlapProps)) overlapper._overlapProps = {};
                if (overlapper._overlapProps && !overlapper._overlapProps.visited) {
    ...
    adjustDimensionsForOverlap : function (eventWin, left, top, width, height) {
        ...
        if (overlapProps && overlapProps.totalSlots > 0) {
            height = Math.floor(height / overlapProps.totalSlots) - this.timelineEventPadding;
            top = top + Math.floor((height * (overlapProps.slotNum - 1)));
            if (overlapProps.slotNum > 1) top += this.timelineEventPadding;
    ...
    getNewEventWindow : function (event) {
        var cal = this.creator,
            ...
            headerProps=isc.addProperties({}, this.headerProperties,
