    ...
    _setContents : function (contents) {
        ...
        if (this._useDesignMode()) {
            ...
            if (handle) {
                ...
                handle.style.width = this.getContentFrameWidth()-1 + "px";
                handle.style.height = this.getScrollHeight()-1 + "px";
            }
        }
...
isc.RichTextItem.addMethods({
    ...
    editAreaFocusChanged : function () {
        if (this.canvas.editArea.hasFocus) this.elementFocus();
    ...
    getValue : function () {
        if (this.canvas && !this._creatingCanvas) this.updateValue();
